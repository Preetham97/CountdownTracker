...
- **App-icon-style progress ring** — Each row carries a miniature of the app icon to the left of its day count: a glowing red trim over a gray track that starts full at the moment the countdown was created and depletes clockwise to empty when the deadline arrives. Completed rows hide the ring; legacy rows that pre-date the per-item `createdAt` field fall back to a 30-day rolling window so they still animate sensibly.
