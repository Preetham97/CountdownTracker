...
- **App-icon-style progress ring** — Each row carries a miniature of the app icon to the right of its day count: a glowing trim over a gray track that starts full at the moment the countdown was created and depletes clockwise to empty when the deadline arrives. Tint mirrors the day-count text (red overdue / red <1d / orange <7d / green ≥7d) so urgency reads identically in either visual element. Completed rows hide the ring; legacy rows that pre-date the per-item `createdAt` field fall back to a 30-day rolling window so they still animate sensibly.
